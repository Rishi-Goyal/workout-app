...
function main() {
  ...
  // v4.6.0 — also emit a per-id asset kind map so the runtime can tell
  // whether a bundled image is actually animated (.gif/.webp) or just a
  // still (.jpg/.png). The Image component renders all of them, but
  // ExerciseGif uses the kind to show the "FIRST POSITION" disclosure
  // when a dynamic exercise's bundled visual is a still frame.
  const kindFor = (file) => {
    const ext = file.split('.').pop().toLowerCase();
    return ext === 'gif' || ext === 'webp' ? 'animated' : 'still';
  };
  const lines = [
    '/**',
    ...
    ...ids.map((id) => `  '${id}': '${kindFor(byId.get(id))}',`),
    '};',
    '',
  ];
